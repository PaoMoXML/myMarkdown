...
final class AnnotationTypeMapping {
   ...
   private Map<Method, List<Method>> resolveAliasedForTargets() {
      ...
      for (int i = 0; i < this.attributes.size(); i++) {
         ...
         AliasFor aliasFor = AnnotationsScanner.getDeclaredAnnotation(attribute, AliasFor.class);
          //如果可以获取到aliasFor注解就执行下一步
         if (aliasFor != null) {
             //方法上定义的aliasFor注解中定义的别名注解的属性（方法）
   ...
   private Method resolveAliasTarget(Method attribute, AliasFor aliasFor, boolean checkAliasPair) {
       //AliasFor中的value和attribute属性不能同时定义，他两是相互别名，所以定义一个就好
      ...
      Class<? extends Annotation> targetAnnotation = aliasFor.annotation();
       //Annotation.class是默认值，如果是默认值，就认为这个属性的别名属性就在该注解内
      ...
      if (!isCompatibleReturnType(attribute.getReturnType(), target.getReturnType())) {
       ...
       //检查是否是别名对
       //递归检查别名属性的是否有别名，如果有别名则进一步检查别名的别名是否是自己（如果checkAliasPair是true，则别名的别名必须是自己）
      if (isAliasPair(target) && checkAliasPair) {
         ...
         if (targetAliasFor != null) {
            ...
            Method mirror = resolveAliasTarget(target, targetAliasFor, false);
             //别名属性的别名必须是自己
             //attribute --AliasFor--> target --AliasFor--> attribute  √
             //attribute --AliasFor--> target --AliasFor--> other attribute  ×
